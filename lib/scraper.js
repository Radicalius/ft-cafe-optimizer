...
import { getNextWeekdayDateString } from './dateUtil';

export async function scrapeMenu() {
    var res = await scrapeCafeMenu('park-east-cafe', 0);
    res = res.concat(await scrapeCafeMenu('park-place-cafe', res.length > 0 ? res[res.length - 1].id : 0));
    return res;
}
...
async function scrapeCafeMenu(cafeName, lastId) {
    ...
    const resp = await fetch(`https://franklintempletonsm.cafebonappetit.com/cafe/${cafeName}/${getNextWeekdayDateString()}/`);
    if (!resp.ok) {
